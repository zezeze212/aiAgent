...
public class ToolRegistry {
	...
	public ToolExecutionResult executeWithResult(String toolName, Map<String, Object> arguments) {
		...
		if (tool == null) {
			...
			return new ToolExecutionResult(toolName, false, errorMessage, errorMessage, costMs);
		}
		...
		try {
			...
			log.info("Agent 工具执行成功，toolName={}, costMs={}, resultLength={}", toolName, costMs,
					result == null ? 0 : result.length());

			return new ToolExecutionResult(toolName, true, result, null, costMs);
		}
		catch (Exception e) {
			long costMs = System.currentTimeMillis() - startTime;
			...
			return new ToolExecutionResult(toolName, false, "工具执行失败：" + errorMessage, errorMessage, costMs);
		}
	}
	...
	public List<ToolInfo> listTools() {
		return toolMap.values()
			.stream()
			.map(tool -> new ToolInfo(tool.name(), tool.description(), tool.parameterSchema()))
			.toList();
	}
