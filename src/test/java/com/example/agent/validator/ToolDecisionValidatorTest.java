...
class ToolDecisionValidatorTest {
	...
	private final ToolDecisionValidator validator = new ToolDecisionValidator();
	...
	void shouldNormalizeValidToolDecision() {
		...
		ToolDecision result = validator.validateAndNormalize(decision);

		assertSame(decision, result);
		assertEquals("analyzeSqlErrorWithSchema", result.getToolName());
